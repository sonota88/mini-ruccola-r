...
run_test_json() {
  ...
  cat $infile | $RUNNER_CMD test_json.R
}

run_lex() {
  local infile="$1"; shift

  cat $infile | $RUNNER_CMD mrcl_lexer.R
}

run_parse() {
  local infile="$1"; shift

  cat $infile | $RUNNER_CMD mrcl_parser.R
}

run_codegen() {
  local infile="$1"; shift

  cat $infile | $RUNNER_CMD mrcl_codegen.R
}
